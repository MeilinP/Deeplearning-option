...
class PolygonOptionsDataLoader:
    """Fetch and cache options data from Polygon.io"""
    ...
    def get_options_chain(self, 
        ...
        Note: Polygon Starter plan provides up to 2 years of historical data.
        For best results, use date ranges within the last 2 years.
        
        Args:
            ticker: Stock ticker symbol (e.g., "AAPL")
            start_date: Start date in YYYY-MM-DD format (max 2 years back)
        ...
        if cache_file.exists() and not force_refresh:
            ...
            with open(cache_file, 'rb') as f:
                ...
                # Only return if not empty, otherwise we might want to try re-fetching
        ...
        try:
            ...
            # Note: list_options_contracts is a paginated response
            ...
            # list_options_contracts returns a generator/iterator
            has_results = False
            ...
            if not has_results:
                ...
                print("Suggestion: Check if your Polygon plan supports historical data for these dates.")
                print("Note: Dates in list_options_contracts refer to expiration dates.")
        
        except Exception as e:
            print(f"Error fetching options data: {e}")
            print("Note: Ensure your POLYGON_API_KEY is correct and has access to this data.")
        
        df = pd.DataFrame(options_data)
        
        # Cache the data only if it's not empty
        if not df.empty:
...
class OptionsFeatureEngineering:
    ...
    def create_straddle_features(call_data: Dict, put_data: Dict, spot_price: float) -> Dict:
        """
        Create features for a straddle (long call + long put at same strike)
...
if __name__ == "__main__":
    ...
    print("\n✓ Data utilities module working correctly!")
